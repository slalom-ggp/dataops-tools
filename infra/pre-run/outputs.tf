output "aws_default_region" { value = local.aws_region }
...
output "ssh_private_key_filenames" {
  value = [local_file.ssh_installed_private_key_path.filename, abspath(module.ssh_key_pair.private_key_filename)]
}
output "ssh_public_key_filenames" {
  value = [local_file.ssh_installed_public_key_path.filename, abspath(module.ssh_key_pair.public_key_filename)]
}
